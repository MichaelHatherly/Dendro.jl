@testset "python profile" begin
    p = Dendro.parser_for(:python)
    prof = Dendro.PROFILES[:python]
    ...
    u2 = only(Dendro.functions(parse(p, "def e():\n    pass\n"), prof))
    @test Dendro.empty_body(u2.node, prof)
