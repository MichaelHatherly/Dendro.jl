@testitem "cpp range-based for is a loop" setup = [Fixtures] tags = [:languages] begin
    ...
    @test Dendro.cyclomatic(u, i) == 3      # base + two range-fors
    @test Dendro.nesting_depth(u, i) == 2
end
...
@testitem "npath per grammar" setup = [Fixtures] tags = [:languages] begin
    @testset "npath $(case.lang)/$(case.name)" for case in Fixtures.NPATH_CASES
        ...
        u = only(Dendro.units(i))
        @test Dendro.npath(u, i) == case.npath
    end
end
...
@testitem "language profiles" setup = [Fixtures] tags = [:languages] begin
    for case in Fixtures.LANGUAGE_CASES
        @testset "$(case.lang) profile" begin
            ...
            @test Dendro.cyclomatic(u, i) == case.cyclomatic
            @test Dendro.cognitive_complexity(u, i) == case.cognitive
            @test Dendro.parameter_count(u, i) == case.params
            @test Dendro.nesting_depth(u, i) == case.nesting
            @test Dendro.function_length(u) == case.length
            @test Dendro.boolean_complexity(u, i) == case.boolean
            @test Dendro.return_count(u, i) == case.returns
