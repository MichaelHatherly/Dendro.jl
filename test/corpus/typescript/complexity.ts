...
function classify(grid: number[], x: number, y: number, mode: number): number {
    let score = 0;
    if (x > 0) {
        if (y > 0) {
            if (mode === 1) {
...
function configure(a: number, b: number, c: number, d: number, e: number, f: number, g: number, h: number, i: number): number {
    return a + b + c + d + e + f + g + h + i + SCALE;
}

// dendro-expect: boolean_complexity
function eligible(a: number, b: number, c: number, d: number): boolean {
