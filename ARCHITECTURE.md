...
- `linkage.jl` defines corpus-wide symbol resolution. `corpus_symbols` builds a
  ...
  predicates (`export_public`, `underscore_public`, `capitalized_public`, `always_public`)
  decide public-API membership, and `public_surface` gives each file its export set, the
  file's own for an import model, the inclusion component's for a splice. Reuses the
  `bindings.jl` capture walk and the `clones.jl` union-find. Included after `naturalness.jl`.
