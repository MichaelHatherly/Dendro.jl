...
A capture names the rule a match reports under, and [Pattern query syntax](@ref) is what
a query file may say: the capture conventions, the predicates available, and how a
repeated shape factors out.
...
described under [Custom rules](@ref). That path is reachable from Julia but never from
