; The shipped pack, realised for TypeScript. `builtin.toml` says what each rule means; this
; file says what it looks like. A rule with no pattern here never fires for TypeScript,
; which is ordinary: the declaration is language-independent and no grammar spells every
; shape.
;
; Five of the pack's rules have no TypeScript pattern, the same five JavaScript leaves out
; and for the same reasons. `nothing_equality` would name `== null`, the deliberate loose
; check that catches `undefined` too. `type_equality` and `length_index_range` name the
; idioms. `redundant_collect` has no standard lazy sequence to materialise, and
; `redundant_default` no `get` with a default to pass.
;
; The grammar is JavaScript's with types layered on, so the shapes read the same. The one
; place it diverges is a class heading: a superclass sits in an `extends_clause`, where
; JavaScript holds it directly in the `class_heritage`.
...
;
; TypeScript names no exception type on a `catch`, so there is no narrow clause to leave
...
((class_declaration
   (class_heritage (extends_clause value: (identifier) @_base))
...
; several say the signature is doing its checking in the body, where no caller reads it and
; the declared types say nothing about it.
((if_statement
   condition: (parenthesized_expression
